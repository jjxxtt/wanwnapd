...
const estimateBuildTime = (beadCount: number): string => {
  const totalMinutes = Math.max(1, Math.round((beadCount * 3) / 60));
...
export default function Home() {
  ...
  const pixelateImage = (imageSrc: string, gridWidthCount: number, gridHeightCount: number, threshold: number, currentPalette: PaletteColor[], mode: PixelationMode) => {
    console.log(`Attempting to pixelate with grid: ${gridWidthCount}x${gridHeightCount}, threshold: ${threshold}, mode: ${mode}`);
  ...
  useEffect(() => {
    if (originalImageSrc && activeBeadPalette.length > 0) {
       const timeoutId = setTimeout(() => {
         if (originalImageSrc && originalCanvasRef.current && pixelatedCanvasRef.current && activeBeadPalette.length > 0) {
           ...
           pixelateImage(originalImageSrc, granularity, canvasHeight, similarityThreshold, activeBeadPalette, pixelationMode);
         } else {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [originalImageSrc, granularity, canvasHeight, similarityThreshold, customPaletteSelections, pixelationMode, remapTrigger]);
  ...
  return (
    ...
    <div className="min-h-screen p-4 sm:p-6 flex flex-col items-center bg-[radial-gradient(circle_at_top_left,#ffe4f2_0,#fff7fb_32%,#eef8ff_70%,#fff_100%)] dark:bg-[radial-gradient(circle_at_top_left,#3b1230_0,#111827_45%,#020617_100%)] font-[family-name:var(--font-geist-sans)] overflow-x-hidden">
      {/* Apply dark mode styles to the header */}
      <header className="w-full max-w-6xl mt-3 mb-6 sm:mt-5 sm:mb-8 relative overflow-hidden rounded-2xl border border-pink-100/80 dark:border-pink-900/50 bg-white/72 dark:bg-gray-950/72 shadow-xl shadow-pink-100/60 dark:shadow-black/30 backdrop-blur">
        ...
        <div className="relative z-20 grid gap-6 px-5 py-6 sm:px-7 sm:py-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
          <div className="text-left">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-pink-200 bg-pink-50/90 px-3 py-1 text-xs font-semibold text-pink-600 dark:border-pink-400/30 dark:bg-pink-950/40 dark:text-pink-200">
              <span className="h-2 w-2 rounded-full bg-pink-400 shadow-[0_0_12px_rgba(244,114,182,0.85)]"></span>
              二次元像素工房
            </div>
            <h1 className="text-4xl font-black leading-tight tracking-normal text-gray-950 dark:text-white sm:text-6xl">
              婉婉拼豆
            </h1>
            <p className="mt-3 max-w-xl text-sm leading-6 text-gray-600 dark:text-gray-300 sm:text-base">
              上传图片，调成柔和明亮的像素拼豆图纸。适合头像、角色小卡、挂件和手作摆台。
            </p>
            <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold">
              <span className="rounded-full bg-pink-100 px-3 py-1 text-pink-700 dark:bg-pink-500/15 dark:text-pink-200">甜系配色</span>
              <span className="rounded-full bg-violet-100 px-3 py-1 text-violet-700 dark:bg-violet-500/15 dark:text-violet-200">色号映射</span>
              <span className="rounded-full bg-sky-100 px-3 py-1 text-sky-700 dark:bg-sky-500/15 dark:text-sky-200">高清导出</span>
              <span className="rounded-full bg-amber-100 px-3 py-1 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200">手作记录</span>
            </div>
          </div>

          <div className="relative mx-auto w-full max-w-sm">
            <div className="absolute -inset-3 rounded-[2rem] bg-gradient-to-br from-pink-200 via-violet-200 to-sky-200 opacity-70 blur-xl dark:from-pink-500/25 dark:via-violet-500/20 dark:to-sky-500/20"></div>
            <div className="relative rounded-[1.5rem] border border-white/80 bg-white p-4 shadow-2xl dark:border-white/10 dark:bg-gray-900">
              <div className="mb-3 flex items-center justify-between text-xs font-semibold text-gray-400">
                <span>PIXEL PREVIEW</span>
                <span>WANWAN</span>
              </div>
              <div className="grid grid-cols-10 gap-1 rounded-2xl bg-pink-50 p-3 dark:bg-gray-950">
                {[
                  '#fff1f2','#fff1f2','#f9a8d4','#f9a8d4','#c4b5fd','#c4b5fd','#bae6fd','#fff1f2','#fff1f2','#fff1f2',
                  '#fff1f2','#f9a8d4','#f472b6','#fb7185','#a78bfa','#818cf8','#7dd3fc','#bae6fd','#fff1f2','#fff1f2',
                  '#f9a8d4','#f472b6','#ffffff','#fb7185','#312e81','#312e81','#ffffff','#7dd3fc','#bae6fd','#fff1f2',
                  '#f9a8d4','#fb7185','#fecdd3','#f472b6','#a78bfa','#818cf8','#bae6fd','#38bdf8','#7dd3fc','#fff1f2',
                  '#fef3c7','#f9a8d4','#f472b6','#f0abfc','#c4b5fd','#a5b4fc','#7dd3fc','#bae6fd','#fde68a','#fef3c7',
                  '#fff7ed','#fef3c7','#f9a8d4','#f472b6','#ffffff','#ffffff','#7dd3fc','#fde68a','#fef3c7','#fff7ed',
                  '#fff7ed','#fff7ed','#fef3c7','#f9a8d4','#f472b6','#7dd3fc','#bae6fd','#fef3c7','#fff7ed','#fff7ed',
                  '#fff7ed','#fff7ed','#fff7ed','#fef3c7','#f9a8d4','#bae6fd','#fef3c7','#fff7ed','#fff7ed','#fff7ed'
                ].map((color, i) => (
                  <span key={i} className="aspect-square rounded-[4px] shadow-sm" style={{ backgroundColor: color }}></span>
                ))}
              </div>
              <div className="mt-4 grid grid-cols-3 gap-2 text-center text-[11px] font-semibold">
                <div className="rounded-xl bg-pink-50 px-2 py-2 text-pink-700 dark:bg-pink-500/10 dark:text-pink-200">上传</div>
                <div className="rounded-xl bg-violet-50 px-2 py-2 text-violet-700 dark:bg-violet-500/10 dark:text-violet-200">调色</div>
                <div className="rounded-xl bg-sky-50 px-2 py-2 text-sky-700 dark:bg-sky-500/10 dark:text-sky-200">导出</div>
              </div>
            </div>
          </div>
        </div>

        {/* Legacy header hidden after redesign */}
        <div className="hidden">
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="relative flex flex-col items-center space-y-3">
              {/* Legacy title block kept hidden after redesign */}
              <div className="relative">
                <h1 className="relative text-4xl sm:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-fuchsia-500 via-violet-500 to-sky-400 tracking-wider drop-shadow-2xl transform hover:scale-105 transition-transform duration-300">
                  婉婉拼豆
                </h1>
              ...
              <div className="relative">
                <h2 className="relative text-xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-sky-500 via-violet-500 via-fuchsia-500 to-pink-400 tracking-widest transform hover:scale-102 transition-all duration-300">
                  二次元拼豆图纸生成器
                  <span className="text-xs font-normal text-pink-400 dark:text-pink-300 tracking-widest ml-1 align-middle">kawaii lab</span>
                </h2>
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowDesktopModal(false)}>
              <div className="relative mx-4 w-full max-w-md rounded-2xl border border-blue-200 dark:border-blue-700 bg-white dark:bg-gray-800 p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">横屏体验更舒展</h3>
                  <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">如果在电脑上使用，可以把窗口调宽一点，画布和色板会更好操作。</p>
                  <div className="mt-5 flex w-full gap-3">
                    ...
                    <button
                      onClick={() => setShowDesktopModal(false)}
                      className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-pink-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-pink-600 transition-colors"
                    >
                      开始制作
                    </button>
                  </div>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2.5 text-xs">
            <span className="group inline-flex items-center gap-1 rounded-full border border-pink-200 bg-white/70 px-3 py-1 text-pink-600 dark:border-pink-400/30 dark:bg-pink-950/30 dark:text-pink-200 font-medium">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5">
                <path fillRule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 011 1v8a1 1 0 01-1 1H4a1 1 0 01-1-1V4zm1 0v8h12V4H4zm-1 12a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" />
              </svg>
              竖屏友好
            </span>
            <span className="rounded-full border border-violet-200 bg-white/70 px-3 py-1 text-violet-600 dark:border-violet-400/30 dark:bg-violet-950/30 dark:text-violet-200 font-medium">二次元配色</span>
            <span className="rounded-full border border-sky-200 bg-white/70 px-3 py-1 text-sky-600 dark:border-sky-400/30 dark:bg-sky-950/30 dark:text-sky-200 font-medium">图纸导出</span>
          </div>
          <p className="mt-2 text-[10px] text-pink-400 dark:text-pink-300">Wanwan Beads Studio</p>
        </div>
      </header>

      {/* Apply dark mode styles to the main section */}
      <main ref={mainRef} className="w-full max-w-6xl flex flex-col items-center space-y-5 sm:space-y-6 relative overflow-hidden">
        ...
        <div
          ...
          className={`group relative overflow-hidden border border-pink-200/80 dark:border-pink-400/20 rounded-2xl p-6 sm:p-8 text-center ${isMounted ? 'cursor-pointer hover:border-pink-300 dark:hover:border-pink-300/40 hover:-translate-y-0.5' : 'cursor-wait'} transition-all duration-300 w-full max-w-2xl flex flex-col justify-center items-center shadow-lg shadow-pink-100/50 dark:shadow-black/20 bg-white/82 dark:bg-gray-950/72 backdrop-blur`}
          style={{ minHeight: '160px' }}
        >
          <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-pink-400 via-violet-400 to-sky-400"></div>
          {/* Icon color */}
          <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 sm:h-12 sm:w-12 text-pink-400 dark:text-pink-300 mb-2 sm:mb-3 transition-transform duration-300 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
             <path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
          </svg>
          {/* Text color */}
          <p className="text-sm sm:text-base font-semibold text-gray-700 dark:text-gray-200">拖放图片到此处，或<span className="font-bold text-pink-500 dark:text-pink-300">点击选择文件</span></p>
          {/* Text color */}
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">支持 JPG, PNG, GIF 图片格式，或 CSV 数据文件</p>
        </div>

        {/* Apply dark mode styles to the Tip Box */}
        {!originalImageSrc && (
          <div className="w-full max-w-2xl bg-white/72 dark:bg-gray-950/60 p-4 rounded-2xl border border-violet-100 dark:border-violet-400/20 shadow-sm backdrop-blur">
            {/* Icon color */}
            <p className="text-xs text-violet-700 dark:text-violet-200 flex items-start">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1.5 flex-shrink-0 text-pink-500 dark:text-pink-300 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              {/* Text color */}
              <span>小贴士：高对比、轮廓清晰的图更适合生成拼豆图纸。头像、Q版角色、小物件会更容易得到可爱的效果。</span>
            </p>
          </div>
        )}
        ...
        {originalImageSrc && (
          <div className="w-full flex flex-col items-center space-y-5 sm:space-y-6">
            {!isManualColoringMode && (
              <section className="w-full max-w-4xl rounded-2xl border border-pink-100 bg-white/88 p-4 shadow-lg shadow-pink-100/40 backdrop-blur dark:border-pink-400/20 dark:bg-gray-950/76">
                <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white">选择画幅</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">先选规格，系统会立即刷新预览并估算豆数和制作时间。</p>
                  </div>
                  <span className="text-xs font-semibold text-pink-500 dark:text-pink-300">当前 {granularity}×{canvasHeight}</span>
                </div>
                  ...
                  {canvasSizePresets.map((preset) => {
                    ...
                    return (
                      <button
                        key={preset.label}
                        type="button"
                        onClick={() => handleSelectCanvasPreset(preset)}
                        className={`rounded-xl border p-3 text-left transition-all ${
                          isSelected
                            ? 'border-pink-400 bg-pink-50 shadow-md shadow-pink-100 dark:border-pink-300/60 dark:bg-pink-950/30'
                            : 'border-gray-200 bg-white hover:border-pink-200 hover:bg-pink-50/60 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-pink-400/30 dark:hover:bg-pink-950/20'
                        }`}
                      >
                        <div className="font-mono text-xl font-black text-gray-950 dark:text-white">{preset.label}</div>
                        <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">约 {beadCount.toLocaleString()} 颗</div>
                        <div className="mt-1 text-xs font-semibold text-violet-600 dark:text-violet-300">{estimateBuildTime(beadCount)}</div>
                      </button>
                    );
                  })}
                </div>
        ...
        {!isManualColoringMode && originalImageSrc && colorCounts && Object.keys(colorCounts).length > 0 && (
          ...
          <div className="w-full max-w-4xl mt-6 bg-white/88 dark:bg-gray-950/76 p-4 rounded-2xl shadow-lg shadow-pink-100/40 dark:shadow-black/20 border border-pink-100 dark:border-pink-400/20 color-stats-panel">
            {/* Title color */}
            <h3 className="text-lg font-semibold mb-1 text-gray-800 dark:text-gray-100 text-center">
              用色清单
            </h3>
            {/* Subtitle color */}
        ...
        {!isManualColoringMode && originalImageSrc && mappedPixelData && gridDimensions && (
            <div className="w-full max-w-4xl mt-4 grid gap-3 sm:grid-cols-2"> {/* Wrapper div */} 
             ...
             <button
                ...
                className={`w-full py-2.5 px-4 text-sm sm:text-base rounded-xl transition-all duration-300 flex items-center justify-center gap-2 bg-white text-sky-700 border border-sky-200 hover:bg-sky-50 dark:bg-gray-950 dark:text-sky-200 dark:border-sky-400/30 dark:hover:bg-sky-950/30 shadow-sm`}
              >
             ...
             <button
                ...
                className={`w-full py-2.5 px-4 text-sm sm:text-base rounded-xl transition-all duration-300 flex items-center justify-center gap-2 bg-white text-violet-700 border border-violet-200 hover:bg-violet-50 dark:bg-gray-950 dark:text-violet-200 dark:border-violet-400/30 dark:hover:bg-violet-950/30 shadow-sm`}
              >
        ...
        {!isManualColoringMode && originalImageSrc && mappedPixelData && (
            <div className="w-full max-w-4xl mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
              <button
                onClick={() => handleDownloadRequest()}
                disabled={!mappedPixelData || !gridDimensions || gridDimensions.N === 0 || gridDimensions.M === 0 || activeBeadPalette.length === 0}
                className="w-full py-3 px-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-sm sm:text-base rounded-xl hover:from-emerald-600 hover:to-teal-600 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-100/60 hover:shadow-xl hover:-translate-y-0.5 disabled:hover:translate-y-0 disabled:hover:shadow-md"
               >
              ...
              <button
                ...
                className="w-full sm:w-32 py-3 px-4 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-gray-700 shadow-sm transition-all hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-200 dark:hover:bg-gray-900"
              >
                下载设置
              </button>
            </div>
      ...
      <footer className="w-full md:max-w-4xl mt-10 mb-6 py-6 text-center text-xs sm:text-sm text-pink-500 dark:text-pink-200 border-t border-pink-100 dark:border-pink-900/60 bg-gradient-to-b from-white/80 to-pink-50/80 dark:from-gray-950 dark:to-pink-950/20 rounded-lg shadow-inner">
        <p className="font-medium">
          婉婉拼豆 &copy; {new Date().getFullYear()} · Wanwan Beads Studio
        </p>
      </footer>
