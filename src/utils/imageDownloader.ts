...
export async function downloadImage({
  ...
  const processDownload = () => {
    ...
    // 计算底部品牌标识区域的高度
    const xiaohongshuAreaHeight = 35; // 为品牌标识预留的底部空间
    ...
    ctx.fillText('婉婉拼豆', titleStartX, mainTitleY);
    ...
    ctx.fillText('二次元拼豆图纸生成器', titleStartX, subTitleY);
    ...
    if (includeStats && colorCounts) {
      ...
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.fillText(`用色清单 / 共 ${totalBeadCount} 颗`, statsPadding, statsY + 8);
